...
my additon not in ps.
A few things stand out for the voice-agent and web-product work ahead. The design constraint that matters most is that this targets elderly, rural, and low-literacy first-time users with zero training, so voice is the primary modality rather than a convenience layer, and every voice interaction needs a touch fallback. Latency and noise robustness matter more than usual since it runs in a crowded hospital environment.



Module A — Conversational multimodal history engine. This is the voice agent core. It conducts an adaptive clinical interview via speech in Hindi, English, and regional languages, with every question also answerable by tapping a touchscreen. It branches dynamically based on the chief complaint (the document cites the SOCRATES framework for pain questioning), detects red-flag emergency symptoms to trigger priority triage, and includes an AYUSH mode capturing Dashavidha Pariksha parameters for Ayurvedic OPDs. The document explicitly names Bhashini / AI4Bharat as the ASR direction for Indian languages and accents, and calls for a dialogue manager constrained by a clinical history ontology plus TTS for audio prompts.